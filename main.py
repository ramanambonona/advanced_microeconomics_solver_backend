...
class StaticSolver:
    ...
    def solve(problem_type: str, utility_type: str, n_goods: int, 
        ...
        try:
            # Convert string parameters to float or sympy symbols
            converted_params = {}
            for k, v in parameters.items():
                try:
                    converted_params[k] = float(v)
                except (ValueError, TypeError):
                    converted_params[k] = sp.symbols(str(v))
            ...
            # Flatten solution list to single dict (take first solution)
            sol_dict = {}
            if solution:
                if isinstance(solution, list):
                    for sol in solution:
                        sol_dict.update({str(k): str(v) for k, v in sol.items()})
                elif isinstance(solution, dict):
                    sol_dict = {str(k): str(v) for k, v in solution.items()}
...
class DynamicSolver:
    ...
    def solve(T: Union[int, str], utility_expr: str, constraint_expr: str,
        ...
        try:
            # Convert beta to float if string
            ...
            is_infinite = (T == "infinite" or T == "oo" or str(T) == "oo")
...
async def generate_latex(request: Request):
    """Generate a LaTeX report for the given module and results"""
    try:
        body = await request.json()
        module = body.get("module", "static")
...
\\usepackage{{geometry}}
\\geometry{{margin=2cm}}
\\title{{Advanced Microeconomics Solver --- {module.capitalize()} Module}}
\\author{{AMS Report}}
\\date{{\\today}}
\\begin{{document}}
\\maketitle
\\section{{Problem}}
Module: {module} \\\\
Type: {problem.get('problem_type', 'N/A')} \\\\
\\section{{Results}}
...
\\end{{document}}"""
        ...
        return Response(
            ...
            headers={'Content-Disposition': f'attachment; filename=solution_{module}.tex'}
        )
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def generate_report(request: Request):
    """Generate a PDF report (returns simple text PDF placeholder)"""
    try:
        body = await request.json()
        module = body.get("module", "static")
        problem = body.get("problem", {})
        ...
        # Simple text-based report content
        report_content = f"AMS Report - {module} module\nProblem: {problem}\nResults: {results}"
        
        from fastapi.responses import Response
        return Response(
            content=report_content.encode('utf-8'),
            media_type='application/pdf',
            headers={'Content-Disposition': f'attachment; filename=rapport_{module}.pdf'}
        )
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def monte_carlo(request: Request):
    """Run a Monte Carlo simulation on Markov chain states"""
    try:
        body = await request.json()
        T = body.get("T", 10)
        S = body.get("S", 2)
        P = body.get("P", [[0.9, 0.1], [0.2, 0.8]])
        ...
        std_path = []
        for t in range(T):
            states_at_t = [path[t] for path in all_paths if t < len(path)]
            mean_path.append(round(sum(states_at_t) / len(states_at_t), 3))
            variance = sum((s - mean_path[-1])**2 for s in states_at_t) / len(states_at_t)
            std_path.append(round(variance**0.5, 3))
        ...
        return JSONResponse(content={
            ...
            "mean_path": mean_path,
            "std_path": std_path,
            "state_paths_sample": all_paths[:5]
        })
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
